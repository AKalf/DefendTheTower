...
public class GUIManager : MonoBehaviour
{
    ...
    public void InformPlayerManaSlider(int amount)
    {
        if (amount == 0)
        {
            playerPullSlider.value = 0;
        }
        else
        {
            playerPullSlider.value = Mathf.Lerp(playerPullSlider.value, amount, Time.deltaTime * Mathf.Abs(playerPullSlider.value - amount));
        }
    }
    public void SetMaxMana(int value) {
        playerPullSlider.maxValue = value;
    }
    ...
    public void InformCoinText(int totalCoins) {
